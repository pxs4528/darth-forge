...
const FileIcon = () => (
    <svg class="w-4 h-4 text-gray-300 shrink-0" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden>
        <path d="M14 3H6a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V9z" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" />
...
const Navbar = (props: Props) => {
    ...
    onMount(() => {
        try {
            ...
            if (saved && files.includes(saved)) setActive(saved);
    ...
    return (
        <>
            ...
            >
                <svg class="w-6 h-6" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden>
                    {open() ? (
            ...
            <aside
                class={`fixed inset-y-0 left-0 z-20 transform transition-transform duration-200 md:static md:translate-x-0 ${open() ? "translate-x-0" : "-translate-x-full"
                    }`}
                aria-hidden={!open()}
            >
                <nav
                ...
                >
                    ...
                    <div class="mt-4 text-[13px] text-gray-300">Selected: <span class="font-medium">{active()}</span></div>
                </nav>
            </aside>
        </>
    );
