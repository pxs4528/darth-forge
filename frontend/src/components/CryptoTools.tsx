  ...
  return (
    <pre class="text-xs font-mono leading-relaxed overflow-auto h-full whitespace-pre">
      <For each={lines()}>
        {(line) => (
          <div>
            <For each={highlightLine(line)}>{(seg) => <span class={seg.cls}>{seg.text}</span>}</For>
          </div>
        )}
      </For>
    </pre>
  ...
  return (
    <div class="space-y-3">
      <p class="text-gray-400 text-xs">
        Bijective character substitution. Trivially broken by frequency analysis — each letter maps
        to a fixed substitute.
      </p>
      <div class="space-y-1">
      ...
      <Show when={result()}>
        <div class="font-mono text-xs space-y-1.5 pt-1">
          <div>
            <span class="text-gray-500">plaintext </span>
            <span class="text-white">{message()}</span>
          </div>
          <div>
            <span class="text-gray-500">ciphertext </span>
          ...
          <Show when={result()!.decrypted !== undefined}>
            <div>
              ...
              <span class="text-green-400">{result()!.decrypted}</span>
            </div>
          </Show>
...
const RsaDemo: Component = () => {
  ...
  return (
    <div class="space-y-3">
      <p class="text-gray-400 text-xs">
        Security: integer factorization hardness. Real RSA uses 2048–4096 bit primes with OAEP
        padding.
      </p>
      ...
      <Show when={result()}>
        {(r) => (
          <div class="font-mono text-xs space-y-1 pt-1">
            ...
            <div class="pt-1">
              <span class="text-gray-500">m={r().message} → c=m^e mod n=</span>
              <span class="text-yellow-300">{r().ciphertext}</span>
            </div>
            <div>
              <span class="text-gray-500">c={r().ciphertext} → m=c^d mod n=</span>
              <span class={r().correct ? "text-green-400" : "text-red-400"}>
                {r().recovered} {r().correct ? "✓" : "✗"}
              </span>
            </div>
          </div>
        )}
...
const EcdhDemo: Component = () => {
  ...
  return (
    <div class="space-y-3">
      <p class="text-gray-400 text-xs">
        Key exchange on secp256k1 (Bitcoin's curve). Security: ECDLP hardness. 256-bit keys ≈
        RSA-3072 security.
      </p>
      <button
        onClick={run}
          ...
          <div class="font-mono text-xs space-y-2 pt-1">
            ...
            {(["alice", "bob"] as const).map((party) => (
              <div>
                ...
                <span class="text-gray-500">priv </span>
                <span class="text-red-400">{r()[party].private} </span>
                <span class="text-gray-500">pub.x </span>
                <span class="text-yellow-300">{r()[party].public.x}</span>
              </div>
            ))}
            <div class="border-t border-gray-800 pt-1 space-y-0.5">
              <div>
                <span class="text-gray-500">alice shared_x </span>
                <span class="text-green-400">{r().alice_shared_x}</span>
              </div>
              <div>
                <span class="text-gray-500">bob shared_x </span>
...
const ElgamalEncDemo: Component = () => {
  ...
  return (
    <div class="space-y-3">
      <p class="text-gray-400 text-xs">
        Probabilistic encryption — same plaintext produces different ciphertext each call. Security:
        ...
        {(r) => (
          <div class="font-mono text-xs space-y-1 pt-1">
            <div class="text-gray-600">
              // p={r().params.p} g={r().params.g}
            </div>
            <div>
              <span class="text-gray-500">public y </span>
              <span class="text-yellow-300">{r().public_key.y}</span>
            </div>
            <div>
              <span class="text-gray-500">m={r().message} → </span>
              <span class="text-yellow-300">
                (c1={r().ciphertext.c1}, c2={r().ciphertext.c2})
              </span>
            </div>
            <div>
              <span class="text-gray-500">decrypted </span>
...
const ElgamalSigDemo: Component = () => {
  ...
  return (
    <div class="space-y-3">
      <p class="text-gray-400 text-xs">
        Authentication + integrity + non-repudiation. Reusing ephemeral k leaks the private key —
        this exact flaw broke Sony PS3 signing (2010).
      </p>
      <div class="space-y-0.5">
...
const StegoDemo: Component = () => (
  <div class="space-y-3">
    <p class="text-gray-400 text-xs">
      LSB steganography hides data in image pixels by overwriting the least significant bit of each
      colour channel. A 1920×1080 RGB image carries ~750 KB hidden — visually indistinguishable from
      the original.
    </p>
    <div class="font-mono text-xs space-y-1.5">
      <div>
        <span class="text-gray-500">capacity </span>
        <span class="text-white">width × height × 3 channels ÷ 8 bits</span>
      </div>
      <div>
        <span class="text-gray-500">delta </span>
        <span class="text-white">≤ 1 per channel (imperceptible)</span>
      </div>
      <div class="pt-1">
        <span class="text-gray-500">attack </span>
        <span class="text-yellow-300">
          chi-squared steganalysis detects LSB patterns statistically
        </span>
      </div>
      <div>
        <span class="text-gray-500">mitigation </span>
        <span class="text-white">encrypt payload with AES-256-GCM before embedding</span>
      </div>
    </div>
  </div>
);
      ...
      <div class="max-w-6xl mx-auto">
        ...
        <div class="terminal-window">
          ...
          <Show when={active() === "elgamal-enc"}>
            <SideBySide id="elgamal-enc">
              <ElgamalEncDemo />
            </SideBySide>
          </Show>
          <Show when={active() === "elgamal-sig"}>
            <SideBySide id="elgamal-sig">
              <ElgamalSigDemo />
            </SideBySide>
          </Show>
