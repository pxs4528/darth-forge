...
const LogViewer: Component<Props> = (props) => {
  ...
  onMount(() => {
    connectWebSocket();
    telemetry.trackInteraction("log-viewer-opened");
  });
  ...
  const clearLogs = () => {
    setLogs([]);
    telemetry.trackClick("log-viewer-clear");
  };

  const togglePause = () => {
    setPaused(!paused());
    telemetry.trackClick("log-viewer-pause", { paused: !paused() });
  };

  return (
    <div class="fixed inset-0 bg-black/80 z-[100] flex items-center justify-center p-4">
      <div class="terminal-window w-full max-w-6xl h-[80vh] flex flex-col">
        {/* Header */}
        <div class="flex items-center justify-between p-4 border-b border-gray-700">
          <div class="flex items-center gap-4">
            <h2 class="text-white font-bold">Live Logs</h2>
            <span
              class={`text-sm ${
          ...
          <div class="flex gap-2">
            ...
            <button
              onClick={() => {
                telemetry.trackClick("log-viewer-close");
                props.onClose();
              }}
              class="px-3 py-1 text-sm border border-white text-white hover:bg-white/20 transition-colors"
            >
